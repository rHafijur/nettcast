...
@section('content')
    ...
    <div class="box">
        ...
        <div class="box-body">
            <div class="row">
                <div class="col-md-6">
                    <div class="input-group">
                        <label>Title</label>
                        <input type="text" value="{{$device->title}}" class="form-control" id="inp_title" placeholder="title">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="input-group">
                        <label>Slug</label>
                        <input type="text" value="{{$device->slug}}" class="form-control" id="inp_slug" placeholder="slug">
                    </div>
                </div>
            </div>
            <div class="row">
                    ...
                    <div class="input-group">
                        ...
                        <select id="brand" class="form-control">
                            ...
                            @foreach ($brands as $brand)
                                <option @if($brand->id==$device->brand_id) selected @endif value="{{$brand->id}}">{{$brand->title}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="input-group">
                        ...
                        <select disabled onchange="optionChanged(this)" id="category" class="form-control">
                            <option value="">Select Category</option>
                            @foreach ($categories as $category)
                                <option @if($category->id==$device->category_id) selected @endif value="{{$category->id}}" data-sp='{{$category->specification_attributes}}' data-cover1='{{$category->cover_specification_attributes_1}}' data-cover2='{{$category->cover_specification_attributes_2}}'>{{$category->title}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="input-group">
                        ...
                        <input type="text" value="{{$device->meta_title}}" class="form-control" id="inp_meta_title" placeholder="title">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="input-group">
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="input-group">
                        ...
                        <textarea name="" id="inp_meta_description" class="form-control"  rows="4">{{$device->meta_description}}</textarea>
                    </div>
                </div>
            </div>
            <div class="row">
            ...
            <form id="submit" action="{{CRUDBooster::mainpath("edit")}}" method="POST">
                @csrf
...
<script>
    var data=`{!!$device->getJsonString("specifications")!!}`;
    // data=data.replace(/\\"/g,'\\\"');
    // data=data.replace(`"`,"");
    var cover_1 = `{!!$device->getJsonString("cover_specifications_1")!!}`;
    var cover_2 = `{!!$device->getJsonString("cover_specifications_2")!!}`;
   var attribute_json;
   var cover1_json;
   var cover2_json;
