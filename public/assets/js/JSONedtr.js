...
function JSONedtr( data, outputElement, config = {} ){
	...
	JSONedtr.level = function ( node, lvl=0 ) {
		...
		$.each( node , function( key, value ) {
			...
			if( typeof value == 'object' ) {
				...
				output += '<div class="jse--row jse--row--array" id="jse--row-'+instanceCount+'-'+ JSONedtr.i + '"><input type="text" onkeyup="jseUpdate(this)" class="jse--key jse--array" data-level="' + lvl + '" value="' + key + '"> : <span class="jse--typeof">(' + type + ')</span>';
				output += JSONedtr.level( value, lvl+1 );
				output += '<div class="jse--delete">✖</div></div>';
			} else {
				if( typeof value == 'string' )
					value = value.replace(/\"/g,"&quot;");
				output += '<div class="jse--row" id="jse--row-'+instanceCount+'-' + JSONedtr.i + '"><input type="text" onkeyup="jseUpdate(this)"  class="jse--key" data-level="' + lvl + '" value="' + key + '"> : <span class="jse--typeof">(' + typeof value + ')</span><input type="text" class="jse--value" value="' + value + '" data-key="' + key + '"><div class="jse--delete">✖</div></div>';
			}
		})
	...
	JSONedtr.getData = function( node = $(JSONedtr.outputElement + ' > .jse--row > input') ){
		// console.log();
		// var _node=$(JSONedtr.outputElement + ' > .jse--row > input');
		var result = {};
		// console.log(node);
		$.each( node, function() {
	...
	JSONedtr.init = function( data, outputElement ) {
		...
		$( outputElement + ' .jse--row input' ).on( 'change input', function( e ){
			if(JSONedtr.config.runFunctionOnKeyUp){
				JSONedtr.executeFunctionByName( JSONedtr.config.runFunctionOnKeyUp , window, JSONedtr);
			}
			if ( JSONedtr.config.runFunctionOnUpdate ) {
				if( JSONedtr.config.instantChange || 'change' == e.type )
					JSONedtr.executeFunctionByName( JSONedtr.config.runFunctionOnUpdate , window, JSONedtr);
...
function jseUpdate(el){
	const val = el.value;
	el= $(el).closest(".jse--row");
	var val_els= $(el).find(".jse--value");
	for(val_el of val_els){
		$(val_el).data('key',val);
	}
// console.log(val_els);
}